...
def get_blobs(img_mask):
    ...
    for j in range(img_mask_height):
        # 	TODO: Iterate through all 'x' coordinates in img_mask
        for i in range(img_mask_width):
            ...
            if img_mask[j][i] == 1:
                exp = expand_nr(mask_copy, (j, i), [])
                # 		TODO: Add blob_coords to blobs_list
                blobs_list.append(exp)
...
def main():
    ...
    # Examples:
    add_color_range_to_detect(dark_blue[::-1], light_blue[::-1])
    add_color_range_to_detect(dark_red[::-1], light_red[::-1])
    add_color_range_to_detect(dark_green[::-1], light_green[::-1])
    add_color_range_to_detect(dark_yellow[::-1], light_yellow[::-1])
